					...
					for (var n = 0; n <= myoptions.dateFormat.length; n++) {
						...
						switch (chr) {
							...
							case "D": //A textual representation of a day, three letters 
								dateStr += new Intl.DateTimeFormat(myoptions.langSet, {
									weekday: 'short'
								}).format(mytimestamp_sysdiff);
								break;
				...
				for (var nn = 0; nn <= myoptions.timeFormat.length; nn++) {
					...
					switch (chrr) {
						...
						case "r": //» RFC 2822 formatted date | Example: Thu, 21 Dec 2000 16:01:07 +0200
							timeStr += new Intl.DateTimeFormat(myoptions.langSet, {
								weekday: 'short'
							}).format(mytimestamp_sysdiff) + ', ' + dt + ' ' + new Intl.DateTimeFormat(myoptions.langSet, {
								month: 'short'
							}).format(mytimestamp_sysdiff) + ' ' + y + ' ' + ('' + h).padStart(2, "0") + ':' + ('' + m).padStart(2, "0") + ':' + ('' + s).padStart(2, "0") + ' ' + (tzH < 0 ? '+' + ('' + Math.abs(tzH)).padStart(2, "0") : (tzh > 0 ? ('' + (tzh * -1)).padStart(2, "0") : "+00")) + "00";
							break;
			...
			if (typeof options === 'undefined' || typeof options === 'object') {
				...
				if (options.timestamp != "localsystime") {
					...
					//OTHERWISE IT'S SIMPLY A CUSTOM JAVASCRIPT TIMESTAMP
						...
						if (options.timezone == "localsystimezone") {
							...
							if (tzOffset < 0) {
								options.timezone += ('+' + Math.abs(tzOffset)) + (suffix !== '' ? ':' + suffix : '');
							} else if (tzOffset > 0) {
								options.timezone += (tzOffset * -1) + (suffix !== '' ? ':' + suffix : '');
							}
						}
				...
				//OTHERWISE IF NO TIMESTAMP HAS BEEN PASSED IN
					...
					if (options.timezone == "localsystimezone") {
						...
						if (tzOffset < 0) {
							options.timezone += ('+' + Math.abs(tzOffset)) + (suffix1 !== '' ? ':' + suffix1 : '');
						} else if (tzOffset > 0) {
							options.timezone += (tzOffset * -1) + (suffix1 !== '' ? ':' + suffix1 : '');
						}
					}
				}
				/*********************************|
